...
//placed here as its only loaded in once, all main stb usages are in this cpp
...
void VulkanBase::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
	...
	allocInfo.memoryTypeIndex = GP2DataBuffer::FindMemoryType(physicalDevice, memRequirements.memoryTypeBits, properties);
...
void VulkanBase::drawFrame(uint32_t imageIndex) {
	...
	//instance
	vkCmdBindPipeline(commandBuffers[currentFrame].GetVkCommandBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, m_GraphicsPipelineInstance.GetPipeline());
	m_GraphicsPipelineInstance.Record(commandBuffers, imageIndex, currentFrame);
